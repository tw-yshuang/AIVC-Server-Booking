    ...
    while True:
        ...
        # using datetime
            ...
            if len(start_time_args) != 5:
                print(str_format(word = f'Wrong input!!! Incorrect number of parameters: expected 5, received {len(start_time_args)}', style = 'default', fore = 'r' , background = 'black'))
                continue
        ...
        if len(week_flag + day_flag) > 1:
            ...
            # using datetime
                ...
                if dif < 0 or dif > 1209600:                        # 1209600 = 2 weeks -> sec
                    ...
                    booking_time.end = datetime.strftime(datetime.fromtimestamp(booking_time.end.timestamp() - 28800), '%Y-%m-%d %H:%M:%S') # for same structure
                    booking_time.start = datetime.strftime(datetime.fromtimestamp(booking_time.start), '%Y-%m-%d %H:%M:%S')
                    break
    # Optional
...
def __user_options(user_id, password):
    ...
    # Part Forward_ports
    while True:
            user_config.forward_port = input(f"Please enter the forward port(default:{checker.users_config.ids[user_id].forward_port}, none by default):\n")
            ...
            if user_config.forward_port < forword_port_begin or user_config.forward_port > forword_port_end:
                print(str_format(word = f'Forward Port must between {forword_port_begin}~{forword_port_end}!', style = 'default', fore = 'r' , background = 'black'))
                continue
    ...
    while True:
        try:
            update_users_config: bool = ask_yn(question = "The previous setting is for the once, do you want to update the default config?", fore = "white")
        except IndexError:
            print(str_format(word = 'ValueError!', style = 'default', fore = 'r' , background = 'black'))
