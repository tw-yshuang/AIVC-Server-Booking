...
> Add `id` or `--user-id` option is to log in system
...
> Use following command to start to update config. Both  `-use-opt` and `--use-options` are acceptable.
...
#### **I. Log in steps**
This system will ask you to input the password and you have two chances to input :

```bash
Password:
```

After that, describe how many resouces the container requiring :
...
All inputs **cannot lower than 1**. Lower numbers are forbidden.

The final step is to select when you want the container and its end time. There are some limits you should know :
...
After system has already checked that computing resources are affordable and there aren't any problems, your booking is successfully.

#### **II. Update Account Setting**


1. The system will ask you whether you want to change forward port. The *forward_port* only can assign port: **10000~11000**, due to application service port. Besides, the system will automatically check whether the *forward_port* you want is duplicated. For more details, go to check [List of TCP and UDP port numbers](https://en.wikipedia.org/wiki/List_of_TCP_and_UDP_port_numbers).
...
3. You can customize your extra commands which will be executed before building container. Customize it to set the environment you want.
...
*( I will give you a brief introduction. If you want to realize more, you can click the name of package to view the detail.)*

### Environment Setting

Every project have its own required packages. The better way to develop is to build a independent environment for each project and let them can setting their own *Spec.*  
